...
std::vector<DominoNode*> getInputDominoes(const std::string& filename);
...
int main() {
    try {
        ...
        std::vector<DominoNode*> inputDominoes = getInputDominoes("dominoes-test_data/10/10-input-uncoloured.txt");
...
std::vector<DominoNode*> getInputDominoes(const std::string& filename) {
    std::vector<DominoNode*> dominoes;
